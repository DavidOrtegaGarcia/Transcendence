...
const EditProfile = () => {
	...
	useEffect(() => {
		/* If is loading, do nothing (wait for auth state to resolve) */
		if (isAuthLoading)
			return;

		/* If there's no authenticated user, redirect to signin */
		if (!authUser) {
			navigate('/signin');
			return;
		}
		const fetchUserData = async () => {
			setIsLoading(true);
			try {
				// USAR ID, que es lo que espera tu API
				const userData = await userService.getProfile(authUser.id);

				/* Mapear el lenguaje de la base de datos al formato que usa i18n */
				...
				// Sincronizar la interfaz visual inmediatamente
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			/* Profile data: Username, Bio and Language (email is required by backend)*/
			...
			/* Profile Password if user changed it */
			if (formData.newPassword) {
			...
			alert(t('errors.update_failed')); // TODO: Mejor manejo de errores (mostrar mensaje específico, etc)
		} finally {
			setIsSaving(false);
		}
	};
	...
	const handleLanguageSelect = (langCode: string) => {
		...
		console.log("Selected language:", langCode, "Mapped to i18n code:", targetLang);
    	i18n.changeLanguage(targetLang);
		console.log("Current i18n language after change:", i18n.language);

		// Cambio inmediato visual
    	localStorage.setItem('lang', targetLang);
	};
